...
const text = /^[a-zA-ZñÑá-úÁ-Ú ]{2,20}/;
const email = /\w+@misena\.edu\.co/;
const passwordd = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*()\-_=+{};:,<.>]).{10,}$/;
...
form.Nom.addEventListener('input', e => {
  ...
  if (text.test(e.target.value)) {
    ...
    f2.textContent = "Solo se aceptan valores alfanumericos (minimo 2 y maximo 20 caracteres)."
...
form.Apl.addEventListener('input', e => {
  ...
  if (text.test(e.target.value)) {
    ...
    f3.textContent = "Solo se aceptan valores alfanumericos (minimo 2 y maximo 20 caracteres)."
...
form.Pass.addEventListener('input', e => {
  ...
  if (passwordd.test(e.target.value)) {
    ...
    f6.textContent = "Ingrese mínimo un un número ,unaletra mayúscula, una letra minúscula y un caracter especial(requiere mínimo 10 caracteres)."
...
form.CoPass.addEventListener('input', e => {
  ...
  if (passwordd.test(e.target.value)) {
    ...
    f7.textContent = "Ingrese mínimo un un número ,una letra mayúscula, una letra minúscula y un caracter especial(requiere mínimo 10 caracteres) y debe coincidir con la contraseña posteriormente digitada."
...
form.addEventListener("submit", e => {
  ...
  if (form.FeNa.value == null || form.FeNa.value == 0 || new Date(form.FeNa.value) < 18) {
  ...
  if (form.CoPass.value == null || form.CoPass.value == 0 || !passwordd.test(form.CoPass.value) || form.CoPass.value != form.Pass.value) {
    alert("Debe ingresar una contraseña que coincida con la ingresada.");
    form.CoPass.focus();
    form.CoPass.setAttribute("class", "error");
    flag = false;
  }
